...
20. Model allowlist — `server.py` validates `request.model` against `ALLOWED_MODELS` frozenset (`claude-sonnet-4-6`, `claude-opus-4-6`, `claude-opus-4-7`, `claude-haiku-4-5-20251001`) before subprocess invocation; unknown models are rejected with HTTP 400 (RR-15, resolved 2026-03-30)
